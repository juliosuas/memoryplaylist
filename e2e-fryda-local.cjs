...
(async () => {
  ...
  await page.getByRole('button', { name: /Feliz/i }).click();
  await page.getByRole('button', { name: /Vacaciones/i }).click();
  await page.screenshot({ path: path.join(outDir, '02-form-filled.png'), fullPage: true });
  await page.getByRole('button', { name: /Generar Playlist/i }).click();
